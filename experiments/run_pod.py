...
_LLAMA = ("experiments/phase3_track1.py --base meta-llama/Meta-Llama-3-8B-Instruct "
          ...
          "--harmless data/alpaca_harmless.jsonl --ks 1 2 5 --seeds 42 1337 --max-new-tokens 128")
...
def preflight(job: dict) -> None:
    ...
    for label, command in job["steps"]:
        ...
        kw = dict(n_extract=a["n_extract"], n_score=a["n_score"])
        if a["matched"]:
            ok, reason = ts.feasibility(matched_path=str(REPO / a["matched"]), **kw)
        else:
            ok, reason = ts.feasibility(advbench_path=str(REPO / a["advbench"]),
                                        harmless_path=str(REPO / a["harmless"]), **kw)
